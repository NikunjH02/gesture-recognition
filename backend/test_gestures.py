...
# WebSocket URL
BACKEND_WS_URL = "http://10.240.39.120:5000/"
    ...
    while True:
        data = {
            ...
            "channels": [
                {
                    "channel": i,
                    "raw": random.randint(45000, 56000),
                    "voltage": round(random.uniform(0, 3.3), 3)
                }
                for i in range(5)
            ]
        }
        ...
        try:
            sio.emit("adc_data", data)
            print(f"Sent data at {data['timestamp']} for device {DEVICE_ID}")
        except Exception as e:
            print(f"Error sending data: {e}")
